...
const checkForADLPoints = adl => {
	if (isEmptyObj(adl)) return 0;
	const dayOfWeek = format(new Date(), "ddd");
	if (!adl.EnabledDaily) {
		return Math.round(adl.PointsTotal / 7);
	}
	return adl[`Points${dayOfWeek}`];
};
