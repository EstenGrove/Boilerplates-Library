...
const markCompleteTracking = (record, currentShiftID) => {
	console.log(`Record from MarkComplete`, record);
	return {
		...record,
		IsCompleted: true,
...
const recordQuickComplete = async (
	...
	if (response) {
		...
		`);
		console.log(`✅ Success! Task was updated`, response);
		return response;
	} else {
		Sentry.captureMessage(`
		RecordQuickComplete: toggleCompleteHandler() (<DailySummaryListItem/>:174)
		...
		`);
		console.log(`❌ Failed! Task was NOT updated`, response);
		return response;
	}
};
...
const exceptionHandler = (vals, task, facilityExceptions = []) => {
	...
	if (!isEmptyVal(vals.exceptionType)) {
		console.log(`✅ Exception!`, vals.exceptionType);
		exceptionLogger(vals, task);
...
const applyUpdatesToTask = (
	...
	console.group("ALL UPDATES: TRACKING RECORD");
	console.log(
		`Scheduled/Unscheduled?:`,
		isScheduledTask(task) ? "Scheduled" : "Unscheduled"
	);
	console.log(`UI Task/Tracking?:`, isUITask(task) ? `UI` : `Tracking Record`);
	console.log("withException:", withException);
	console.log("withReassess:", withReassess);
	console.log("withNotes:", withNotes);
	console.log("withRepeat:", withRepeat);
	console.log("withShift:", withShift);
	console.log("withStatus:", withStatus);
	console.log("withPastDue:", withPastDue);
	console.groupEnd();
	return withPastDue;
};
